...
- **Step 7 — Bronze Age arc** (content expansion) — new cards + missions themed to the Bronze
  ...
  **Scale — stated plainly, not trimmed:** ≈13 critical nodes + 1 leaf + 1 infinite ≈ **15 missions**,
  ...
  2. **Masonry** — DONE, balance included (see [`missions/masonry.md`](missions/masonry.md)). Optional **Pyramid** leaf — DONE, balance included
  ...
  **Per-node reward proposals** — unbuilt nodes only; shipped nodes live in the status board +
  ...
  - **Fall of the Bronze Age** (infinite, scored survival) — decided, see
    [missions/fall-of-bronze.md](missions/fall-of-bronze.md): the Long Storm feeds endless
    `endless_raid`s into the deck; scored by **waves repelled** (the objective-card `score` seam), not
    rounds survived. Sim-measured through that same measure, feel-play signed off.
