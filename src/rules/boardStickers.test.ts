...
describe('buyBoardSticker', () => {
  it('attaches the sticker and deducts its cost', () => {
    const result = buyBoardSticker({}, 5, TEST_BOARD_ID, 'test_bs_food');
  ...
  it('does not mutate the input map', () => {
    ...
    buyBoardSticker(map, 5, TEST_BOARD_ID, 'test_bs_food');
    expect(map[TEST_BOARD_ID]).toBeUndefined();
  });

  it('appends a second sticker to a board that already has one', () => {
    const first = buyBoardSticker({}, FIXTURE_BOARD_STICKERS.test_bs_food.cost + FIXTURE_BOARD_STICKERS.test_bs_territory.cost, TEST_BOARD_ID, 'test_bs_food')!;
    const second = buyBoardSticker(first.boardStickers, first.influence, TEST_BOARD_ID, 'test_bs_territory');
  ...
  it('allows attaching the same sticker twice — it stacks', () => {
    const first = buyBoardSticker({}, 10, TEST_BOARD_ID, 'test_bs_food')!;
    const second = buyBoardSticker(first.boardStickers, first.influence, TEST_BOARD_ID, 'test_bs_food');
  ...
  it('rejects a third sticker once the board is full', () => {
    const first = buyBoardSticker({}, 20, TEST_BOARD_ID, 'test_bs_food')!;
    const second = buyBoardSticker(first.boardStickers, first.influence, TEST_BOARD_ID, 'test_bs_territory')!;
    expect(buyBoardSticker(second.boardStickers, second.influence, TEST_BOARD_ID, 'test_bs_military')).toBeNull();
  });

  it('rejects an unknown sticker id', () => {
    expect(buyBoardSticker({}, 5, TEST_BOARD_ID, 'not-a-sticker')).toBeNull();
  });

  it('rejects an unaffordable purchase', () => {
    expect(buyBoardSticker({}, 1, TEST_BOARD_ID, 'test_bs_food')).toBeNull();
  });

  it('keeps other boards untouched', () => {
    const first = buyBoardSticker({}, 10, TEST_BOARD_ID, 'test_bs_food')!;
    const second = buyBoardSticker(first.boardStickers, first.influence, TEST_BOARD_2_ID, 'test_bs_military')!;
