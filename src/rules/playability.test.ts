...
// Every requirement here is exercised against a locally-built `CardDef`/`CardInstance` —
// `unplayableReason` is only ever handed the card object directly, never a CARDS[id] lookup — so the
// shipped catalogue doesn't matter for most of these. The one real catalogue dependency is the sticker
// test: a sticker id is looked up live via `effectiveCost`, so it needs a fixture sticker installed.
...
describe('unplayableReason', () => {
  it('is playable when every requirement passes', () => {
  ...
  it('reports the resources still missing when unaffordable', () => {
    const G = blankState('test');
    G.resources.food = 1;
    const card: CardDef = { ...baseCard, cost: { resources: { food: 3, production: 2 } } };
  ...
  it('gates on culture level requirement', () => {
    ...
    const card: CardDef = { ...baseCard, cost: { cultureLevelReq: 1 } };
  ...
  it("returns a card's bespoke cost.check reason (e.g. a recover card needs a non-empty discard)", () => {
    ...
    const card: CardDef = {
      ...baseCard,
      cost: { check: ({ G }) => (G.discard.length === 0 ? { kind: 'discardEmpty' } : null) },
    };
    expect(unplayableReason(G, card, self)).toEqual({ kind: 'discardEmpty' });
  });

  it('composes cost.check with the declarative fields, running check after them', () => {
    // A card declaring both a cultureLevelReq and a check: the declarative field is checked first, and
    // the check only runs (and can block) once every declarative one has passed.
    const card: CardDef = {
      ...baseCard,
      cost: {
        cultureLevelReq: 1,
    ...
    const belowCulture = blankState('test'); // culture 0 → declarative blocks, check not reached
    ...
    cultureMet.discard = [{ id: 1, cardId: 'test' }]; // now both requirements pass
  ...
  it('checks requirements in priority order (price before culture level)', () => {
    const G = blankState('test');
    const card: CardDef = { ...baseCard, cost: { resources: { food: 5 }, cultureLevelReq: 1 } };
  ...
  it("a cost-cutting sticker discounts this copy's cost by 1 per resource, floored at 0", () => {
    const G = blankState('test');
    G.resources.food = 2;
    const card: CardDef = { ...baseCard, cost: { resources: { food: 2, production: 1 } } };
