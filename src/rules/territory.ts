 ...
 *  (`population.ts`'s `nextInstanceId`) and the UI's box lookup fold over it, so a new board zone
 *  reaches them by landing here. Deliberately *not* what the territory cap measures: work boxes and
 *  routes stand on the board without costing land (see `usedTerritory`). Not `threats`/`objective`
 *  either — those are mission pressure standing beside the board, not the player's own cards. */
