...
describe('ageColSpans', () => {
  ...
  it('ignores standard missions missing a map or an age', () => {
    const noMap = { ...mk('a', 'stone', 0), map: undefined };
    const noAge = { ...mk('b', 'stone', 1), age: undefined };
  ...
  it('gives a single age one slice spanning from column 0 past its furthest mission', () => {
    ...
    const spans = ageColSpans([mk('a', 'stone', 2), mk('b', 'stone', 4)]);
  ...
  it('tiles present ages contiguously in AGES order, absorbing gaps between them', () => {
    // Deliberately out of input order; stone 0-1, bronze 3-4 (gap at col 2), iron 6.
    ...
    expect(spans.map((s) => s.age.id)).toEqual(['stone', 'bronze', 'iron']);
    expect(spans).toEqual([
      { age: AGES[0], startCol: 0, endCol: 3 }, // stone → up to bronze's first column
