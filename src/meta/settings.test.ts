...
describe('parseSettings', () => {
  it('shows both tutorial popups to a profile stored before the fields existed', () => {
  ...
  it('keeps a stored dismissal', () => {
    const parsed = parseSettings({ uiScale: 1, theme: 'light', seenRunIntro: true, seenMetaIntro: true });
...
describe('resetTutorialFlags', () => {
  it('owes both tutorials again but keeps every other preference', () => {
    const seen = {
      ...DEFAULT_SETTINGS,
      theme: 'dark' as const,
      uiScale: 1.2,
    ...
    expect(resetTutorialFlags(seen)).toEqual({ ...seen, seenRunIntro: false, seenMetaIntro: false });
  });
});
