...
export const PROGRESS: Record<string, (G: GameState) => number> = {
  ...
  // "Growing Numbers": stand up a Hut and a Farm at once — two distinct buildings, each needing a free
  // territory *slot*, and the Tribe board starts at territory 0. So territory (grown via Conquest) is a
  // genuine prerequisite for both, not a detour. The building term counts *distinct* required types present
  // (so a duplicate Farm earns nothing, pushing the policy toward the type it still lacks); blending
  // territory in as a sub-goal is what steers a one-ply policy to play Conquest at all (a flat building-count
  // gradient never rewards it, since Conquest raises no building on its own turn). Both terms cap at 2 and
  // average ⇒ 1 exactly at the win (two buildings occupy two slots, so territory ≥ 2).
